...
<script type="text/javascript">
	...
	function showHistoryList() {
		$.ajax({
			url:"workbench/transaction/getHistoryList",
			data:{
				"tranId":"${t.id}"
			},
			type:"get",
	...
	function changeStage(stage,i) {
		$.ajax({
			url:"workbench/transaction/changeStage",
			data:{
				"id":"${t.id}",
				"stage":stage,
				"money":"${t.money}",
	...
	function changeIcon(stage,index) {
		...
		if (currentPossibility == "0") {
			...
			// 如果当前阶段的可能性不为0，前7个绿圈、绿色标记、黑圈，后两个黑叉
		} else {
			// 遍历前7个
			for (var i=0;i< point;i++){
				if (i == index){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			if ("0".equals(currentPossibility)){
				for (int i=0;i<dvList.size();i++){
					...
					String stage = dv.getValue();
					String possibility = pMap.get(stage);
					// 如果遍历出来的阶段的可能性为0，说明是后两个，一个为红叉，一个黑叉
					if ("0".equals(possibility)){
						// 如果是当前阶段
		...
		<%
			...
			// 如果当前阶段可能性不为0，前7个有可能是绿圈，绿色标记，黑圈，后两个是黑叉
			}else {
				// 准备当前阶段的下标
				int index = 0;
				for (int i=0;i<dvList.size();i++){
